...
def train(epoch,net,net2,optimizer,labeled_trainloader, unlabeled_trainloader, all_train_loss, all_train_loss_x, all_train_loss_u, savelog=False):
    ...
    while(cont_iters<max_iters): #longmix 
        for batch_idx, (inputs_x, inputs_x2, inputs_x3, inputs_x4, labels_x, w_x, indices_x) in enumerate(labeled_trainloader):
            ...
            if args.sim_clr:
                ...
                #f3, _ = net(inputs_x3, mode = "encoder")
                #f4, _ = net(inputs_x4, mode = "encoder")
                ...
                features_u = torch.cat([f1.unsqueeze(1), f2.unsqueeze(1)], dim=1)
                #features_x = torch.cat([f3.unsqueeze(1), f4.unsqueeze(1)], dim=1)
                loss_simCLR = contrastive_criterion(features_u)
    ...
    print("loss sim clr ", train_loss_simclr, "loss ", train_loss_lx)
...
def eval_train(model, all_loss, all_preds, all_hist, all_margins_labels, eval_loss_hist, eval_acc_hist, clean_labels, net_idx, savelog=False):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets, index) in enumerate(eval_loader):
            ...
            f, outputs = model(inputs, mode = "encoder")
            ...
            vac, entr, diss, margin, evide = compute_dirichlet_metrics(outputs, args.num_class, args.edl_activation, args.evidence_factor)
    ...
    return prob, all_loss, all_preds, all_hist, all_margins_labels, eval_loss_hist, eval_acc_hist, correct_indices, noisy_correct_indices


def get_superclean(prob, net_idx):
...
def write_log(idx, predicted, log_file, threshold = None):
    acc1, acc2 = get_metrics(predicted, clean_labels, idx)
    ...
    log_file.write(f"Accuracy of superclean instances (clean vs noisy): {clean_accuracy:.2f}%\n")
    log_file.write(f"Accuracy of the superclean instances (true class): {acc1:.2f}%\n")
    log_file.write(f"Accuracy of the complimentary: {acc2:.2f}%\n")
    log_file.write("-" * 50 + "\n")
    log_file.flush()
...
def get_metrics(predicted, clean_labels, idx):
    ...
    if len(idx) > 0:
        correct_union = (predicted[idx] == clean_labels_tensor[idx]).sum().item()
        acc = correct_union / len(idx)

    # Compute accuracy for the complement
    comp_indices = torch.tensor([i for i in range(total_samples) if i not in idx])
    if len(comp_indices) > 0:
        correct_comp = (predicted[comp_indices] == clean_labels_tensor[comp_indices]).sum().item()
        acc_comp = correct_comp / len(comp_indices)
    return 100*acc, 100*acc_comp
...
for args in a:
    ...
    for epoch in range(resume_epoch, args.num_epochs+1):   
        ...
        if epoch<warm_up:       
            ...
            prob_loss1, all_loss[0], all_preds[0], hist_preds[0], all_margins_labels[0], eval_loss_hist[0], eval_acc_hist[0], correct_indices1, noisy_correct_indices1 = eval_train(net1, all_loss[0], all_preds[0], hist_preds[0], all_margins_labels[0], eval_loss_hist[0], eval_acc_hist[0], clean_labels, net_idx = 0)
            prob_loss2, all_loss[1], all_preds[1], hist_preds[1], all_margins_labels[1], eval_loss_hist[1], eval_acc_hist[1], correct_indices2, noisy_correct_indices2 = eval_train(net2, all_loss[1], all_preds[1], hist_preds[1], all_margins_labels[1], eval_loss_hist[1], eval_acc_hist[1], clean_labels, net_idx = 1) 
            pred_loss1, threshold_loss_1, thr1_labeled, thr1_unlabeled = get_superclean(prob_loss1, net_idx = 0)
            pred_loss2, threshold_loss_2, thr2_labeled, thr2_unlabeled = get_superclean(prob_loss2, net_idx = 1)
            ...
            remove_log.write(f'mean margins unlabeled : \n{thr1_unlabeled}\n')
            relabeled_log.write(f'mean margins labeled : \n{thr1_labeled}\n') 
            write_log(relabel_idx_1, predicted_labels1, log_file=relabeled_log)
