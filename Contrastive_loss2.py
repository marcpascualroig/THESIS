...
class SupConLoss_old(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        if labels is not None and mask is not None:
            ...
            if labels.shape[0] != batch_size:
                raise ValueError('Num of labels does not match num of features', labels.shape, batch_size)
...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None, sample_weights=None):
        """Compute loss for model. Supports optional sample_weights."""
        ...
        if sample_weights is not None:
            sample_weights = sample_weights.view(-1).to(device)
            if anchor_count > 1:
                sample_weights = sample_weights.repeat(anchor_count)
