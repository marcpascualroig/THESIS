...
def train(epoch,net,net2,optimizer,labeled_trainloader,unlabeled_trainloader, savelog=False):
    ...
    while(cont_iters<max_iters): #longmix 
        for batch_idx, (inputs_x, inputs_x2, labels_x, w_x) in enumerate(labeled_trainloader):      
            try:
                inputs_u, inputs_u2 = next(unlabeled_train_iter)  # Use next() on the iterator
            except StopIteration:
                # If the iterator is exhausted, recreate the iterator
                unlabeled_train_iter = iter(unlabeled_trainloader)
            ...
            loss = Lx + lamb * Lu  +  args.yespenalty*penalty
...
class SemiLoss(object):
    #criterion(logits_x, mixed_target[:batch_size*2], logits_u, mixed_target[batch_size*2:], epoch+batch_idx/num_iter, warm_up)
...
name_exp = '23_01_BASELINE_longremix_stage2_penalty%d_cn%d'%(args.yespenalty, args.num_clean)
