...
class tiny_imagenet_dataset(Dataset):
    def __init__(self, log, root, transform, mode, ratio, noise_mode, noise_file = '', num_samples=10000, pred=[], probability=[], paths=[], num_class=200):
        ...
        if isinstance(noise_label, np.ndarray) and noise_label.shape == ():  
            noise_label = noise_label.item()  # unwrap the dict

        # Now noise_label is either a dict or an array/list of ints
        if isinstance(noise_label, dict):
            # Convert to array aligned with train_images
            self.noise_label = np.array([noise_label[x] for x in self.train_images]).astype(np.int64)
        else:
            self.noise_label = np.array(noise_label).astype(np.int64)

        # Also store train labels
...
class tinyImagenet_dataloader():  
    def __init__(self, root, batch_size, num_workers, log, r,  noise_mode, noise_file):    
    ...
    def run(self,mode,pred=[],prob=[],paths=[]):        
        if mode=='warmup':
            warmup_dataset = tiny_imagenet_dataset(self.log ,self.root,transform=self.transforms["warmup"], mode='all', ratio = self.ratio, noise_mode = self.noise_mode, noise_file=self.noise_file)
            ...
            labeled_dataset = tiny_imagenet_dataset(self.log ,self.root,transform=self.transforms["labeled"], mode='labeled',  ratio = self.ratio, noise_mode = self.noise_mode, noise_file=self.noise_file, pred=pred, probability=prob,paths=paths)
            ...
            unlabeled_dataset = tiny_imagenet_dataset(self.log ,self.root,transform=self.transforms["unlabeled"], mode='unlabeled',  ratio = self.ratio, noise_mode = self.noise_mode, noise_file=self.noise_file, pred=pred, probability=prob,paths=paths)
            ...
            eval_dataset = tiny_imagenet_dataset(self.log ,self.root,transform=self.transform_test, mode='all', ratio = self.ratio, noise_mode = self.noise_mode, noise_file=self.noise_file)
            ...
            test_dataset = tiny_imagenet_dataset(self.log ,self.root,transform=self.transform_test, mode='test', ratio = self.ratio, noise_mode = self.noise_mode, noise_file=self.noise_file)
            ...
            val_dataset = tiny_imagenet_dataset(self.log ,self.root,transform=self.transform_test, mode='val', ratio = self.ratio, noise_mode = self.noise_mode, noise_file=self.noise_file)
