...
def train(epoch,net,net2,optimizer,labeled_trainloader, unlabeled_trainloader, all_train_loss, all_train_loss_x, all_train_loss_u, all_train_loss_contrastive, op, vacuity, margin, dissonance, savelog=False):
    ...
    while(cont_iters<max_iters): #longmix 
        for batch_idx, (inputs_x, inputs_x2, inputs_x3, inputs_x4, labels_x, w_x, indices_x) in enumerate(labeled_trainloader):
            ...
            elif args.plr == "unsupervised":
                bs_x = inputs_x.size(0)
                bs_u = inputs_u.size(0)
                all_inputs = torch.cat([inputs_u, inputs_u2], dim=0)
                all_features = net(all_inputs, forward_pass='proj')
                if all_features.size(0) != 2*bs_u:
                    print("Error: Mismatch in total feature size!")
                    exit(1)
                
                eval_outputs = op[indices_u, :]
                labels_u = labels_u.cuda()
                
                contrastive_mask = Contrastive_loss.build_mask_step(eval_outputs, topK, labels_u, device) 
                
                fu3 = all_features[0:bs_u]
                fu4 = all_features[bs_u:2*bs_u]
                all_features = torch.cat([fu3.unsqueeze(1), fu4.unsqueeze(1)], dim=1)
                weights_all =None
                loss_plr = plr_loss(all_features, mask=contrastive_mask, sample_weights = weights_all) #unsupervised loss: positive examples are 2 augmentations of the same sample only. Negative examples are samples that do no share the same class between the topk predicted classes
            ...
            if args.mixclr:
                # mixmatch
                all_inputs_1 = torch.cat([inputs_x, inputs_u], dim=0)
                all_inputs_2 = torch.cat([inputs_x2, inputs_u2], dim=0)
                ...
                all_features_1 = net(mixed_input_1, forward_pass='proj')
                all_features_2 = net(mixed_input_2, forward_pass='proj')

                features = torch.cat([all_features_1.unsqueeze(1), all_features_2.unsqueeze(1)], dim=1)
            ...
            cl = args.lambda_plr*loss_plr + 0.025*loss_mixclr
            contrastive_loss += cl
            if cl == 0:
...
def eval_train2(model, clean_labels, net_idx, savelog=False):
    model.eval()
    predicted_class = torch.zeros(len(eval_loader.dataset))
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets, index) in enumerate(eval_loader):
            ...
            for b in range(inputs.size(0)):
                predicted_class[index[b]]=pred[b]
...
def eval_train(model, clean_labels, net_idx, save_tensors = True, savelog=False):
    ...
    if save_tensors:
        ...
        eval_acc_hist[net_idx].append([train_acc/len(eval_loader.dataset), acc_clean/len(inds_clean), acc_noisy/len(inds_noisy)])
...
def create_model():
    model = ResNet18(match_checkpoint = False, num_classes=args.num_class)

    if args.use_pretrained:
        #checkpoint = torch.load(f'../experiments2/pretrained/all4one-cifar10-preresnet18-2048_4096_256.ckpt')

        checkpoint = torch.load(
            '../experiments2/pretrained/all4one-cifar10-preresnet18-2048_4096_256.ckpt',
            map_location='cuda:0'     # or 'cpu' if you prefer
        )
...
def save_models(save_path):
    ...
    if epoch%10==0 or epoch ==4 or epoch ==9:
...
for args in a:
    ...
    path_exp='./checkpoint_cifar10/' + exp_str
    ...
    model_path = "./checkpoint_cifar10/%s/model_ckpt.pth.tar"%(exp_str)
    ...
    if incomplete == False:
        ...
        log_mode = 'a'
    stats_log=open('./checkpoint_cifar10/%s/%s_%.2f_%s'%(exp_str, args.dataset,args.r,args.noise_mode)+'_stats.txt',log_mode) 
    test_log=open('./checkpoint_cifar10/%s/%s_%.2f_%s'%(exp_str,args.dataset,args.r,args.noise_mode)+'_acc.txt',log_mode) 
    time_log=open('./checkpoint_cifar10/%s/%s_%.2f_%s'%(exp_str, args.dataset,args.r,args.noise_mode)+'_time.txt',log_mode) 
    superclean_log= open('./checkpoint_cifar10/%s/%s_%.2f_%s'%(exp_str, args.dataset,args.r,args.noise_mode)+'_superclean.txt',log_mode)
    ...
    loader = dataloader.cifar_dataloader(args.dataset,r=args.r,noise_path=args.noise_path,noise_mode=args.noise_mode,batch_size=args.batch_size,num_workers=5,\
    ...
    for epoch in range(resume_epoch, args.num_epochs+1):   
        ...
        if "reduce_lr" in args.name_exp and epoch >= 5:
             lr /= 10
        if 160 > epoch >= 80:
            lr /= 10
        elif epoch >= 160:
        ...
        if epoch<warm_up:       
            ...
            predicted_labels = eval_train2(net1, clean_labels, net_idx=0, savelog=False)     
            correct_indices1 = np.where(clean_labels == predicted_labels)[0]
            print("accuracy ", len(correct_indices1)/len(predicted_labels))
            ...
            if epoch == args.epoch_relabel:
                ...
                all_idx_view_relabel_unlabeled[0].append(relabel_indices(diss1, pred_loss1, threshold=0.02, net_idx=0))
                all_idx_view_relabel_unlabeled[1].append(relabel_indices(diss2, pred_loss2, threshold=0.02, net_idx=1))
                
                print("number of relabeled indices net 1 and net 2: ", len(all_idx_view_relabel_unlabeled[0][-1]), len(all_idx_view_relabel_unlabeled[1][-1]))
                predicted_labels1 = torch.argmax(hist_preds[0][-1], dim=1)
                predicted_labels2 = torch.argmax(hist_preds[1][-1], dim=1)
                correct_relabel = (predicted_labels1[all_idx_view_relabel_unlabeled[0][-1]] == clean_labels[all_idx_view_relabel_unlabeled[0][-1]]).sum().item()
                acc_unlabeled = 100 * correct_relabel / len(all_idx_view_relabel_unlabeled[0][-1])
                print("accuracy relabeled: ", acc_unlabeled)
                relabel_idx_1 = list(set(relabel_idx_1) | (set(all_idx_view_relabel_unlabeled[0][-1])))
                new_labels_1 = predicted_labels1[relabel_idx_1].to(torch.int64)
                relabel_idx_2 = list(set(relabel_idx_2) | (set(all_idx_view_relabel_unlabeled[1][-1])))
                ...
                acc_unlabeled = 100 * correct_relabel / len(all_idx_view_relabel_unlabeled[0][-1])
                print("accuracy relabeled: ", acc_unlabeled)
                correct_relabel = (predicted_labels2[all_idx_view_relabel_unlabeled[1][-1]] == clean_labels[all_idx_view_relabel_unlabeled[1][-1]]).sum().item()
                acc_unlabeled = 100 * correct_relabel / len(all_idx_view_relabel_unlabeled[1][-1])
                print("accuracy relabeled: ", acc_unlabeled)
